    ...
    public static String[] solution(String[] goods) {
        ...
        for(int i=0; i<goods.length; i++) {
            for(int k=1; k<=goods[i].length(); k++) {
                ...
                if (!answer.get(i).isEmpty() && answer.get(i).get(0).length() < k) break;

                for(int j=0; j<=goods[i].length()-k; j++) {
                    String str = good.substring(j, j+k);
                    if (answer.get(i).contains(str)) continue;
                    if (checkString(i, str, goods)) answer.get(i).add(str);
                }
